using Cetas.DbMigration.Core.ComponentModel;
        ...
        private static void Initialize()
        {
            ...
            using (new ReaderWriteLockDisposable(_locker))
            {
                ...
                var compatibilities = typeFinder.FindClassesOfType<INameCompatibility>()
                    ?.Select(type => new CetasEngine().ResolveUnregistered(type) as INameCompatibility).ToList() ?? new List<INameCompatibility>();

                compatibilities.AddRange(AdditionalNameCompatibilities.Select(type => new CetasEngine().ResolveUnregistered(type) as INameCompatibility));
